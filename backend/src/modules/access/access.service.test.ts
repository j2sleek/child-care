...
vi.mock('./childAccess.model.ts', () => ({
  default: {
    findOne: (...args: any[]) => mockAccessFindOne(...args),
    findOneAndUpdate: (...args: any[]) => mockAccessFindOneAndUpdate(...args),
...
import { inviteUserToChild, inviteByEmail, listAccessForChild } from './access.service.ts';
...
describe('access.service', () => {
  ...
  describe('inviteUserToChild', () => {
    ...
    it('should throw FORBIDDEN when inviter has no access', async () => {
      ...
      await expect(
        inviteUserToChild(inviterId, childId, targetId, 'nanny', {
          canRead: true, canWrite: false, canInvite: false,
        })
