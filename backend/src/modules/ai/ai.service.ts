...
import { buildAiContext, collectChildContext, collectRecentEvents, collectNorms, type AiContext } from './ai.collector.ts';
import AiDigestModel from './aiDigest.model.ts';
...
function formatContextForPrompt(ctx: AiContext): string {
  ...
  const sleepSummary = analytics.sleepSummary
    .map((d: any) => `  ${d._id}: ${d.totalSleepMinutes} min (${d.sessions} sessions)`)
    .join('\n') || '  No sleep data';

  const feedingSummary = analytics.feedingPattern
    .map((d: any) => `  ${d._id.day}: ${d.totalAmount ?? 0} ml (${d.feedCount} feeds)`)
    .join('\n') || '  No feeding data';

  const diaperCount = recentEvents.diaper.length;
  const moodEntries = recentEvents.mood
    .map((e: any) => `  ${new Date(e.startTime).toISOString().slice(0, 10)}: ${e.data?.mood ?? e.notes ?? 'logged'}`)
    .join('\n') || '  No mood data';
...
const RAW_RESPONSE_MAX_LEN = 32768;
...
export async function invalidateAiCache(childId: string): Promise<void> {
  try {
    const keys = [
      `ai:insights:${childId}`,
...
export async function generateInsights(userId: string, childId: string) {
  ensureAiAvailable();
...
export async function generateRecommendations(userId: string, childId: string) {
  ensureAiAvailable();
  await requirePermission(userId, childId, 'canRead');
...
export async function detectAnomalies(userId: string, childId: string) {
  ensureAiAvailable();
  await requirePermission(userId, childId, 'canRead');
  ...
  if (norms.sleepMinutesPerDay && analytics.sleepSummary.length > 0) {
    const avgSleep = analytics.sleepSummary.reduce((sum: number, d: any) => sum + d.totalSleepMinutes, 0) / analytics.sleepSummary.length;
    if (avgSleep < norms.sleepMinutesPerDay.low) flags.push(`Average daily sleep (${Math.round(avgSleep)} min) is below the norm range (${norms.sleepMinutesPerDay.low}-${norms.sleepMinutesPerDay.high} min)`);
    if (avgSleep > norms.sleepMinutesPerDay.high) flags.push(`Average daily sleep (${Math.round(avgSleep)} min) is above the norm range (${norms.sleepMinutesPerDay.low}-${norms.sleepMinutesPerDay.high} min)`);
  }
  if (norms.feedsPerDay && analytics.feedingPattern.length > 0) {
    const avgFeeds = analytics.feedingPattern.reduce((sum: number, d: any) => sum + d.feedCount, 0) / analytics.feedingPattern.length;
    if (avgFeeds < norms.feedsPerDay.low) flags.push(`Average daily feeds (${avgFeeds.toFixed(1)}) is below the norm range (${norms.feedsPerDay.low}-${norms.feedsPerDay.high})`);
    if (avgFeeds > norms.feedsPerDay.high) flags.push(`Average daily feeds (${avgFeeds.toFixed(1)}) is above the norm range (${norms.feedsPerDay.low}-${norms.feedsPerDay.high})`);
  }
...
export async function getDailyDigest(userId: string, childId: string, date?: string) {
  ensureAiAvailable();
  await requirePermission(userId, childId, 'canRead');
...
async function generateAndStoreDigest(userId: string, childId: string, date: string) {
  const ctx = await buildAiContext(userId, childId);
  const contextText = formatContextForPrompt(ctx);
...
Scores should be 0-100 based on how well the child's metrics align with age-appropriate norms (100 = perfectly aligned).`;
  ...
  const digest = await AiDigestModel.findOneAndUpdate(
    { childId, date },
    {
      childId,
      date,
      ...
      rawResponse: raw.slice(0, RAW_RESPONSE_MAX_LEN)
    },
...
export async function generateDailyDigest(childId: string): Promise<void> {
  ensureAiAvailable();
...
Generate a comprehensive daily digest. Return JSON with this exact structure:
{
  ...
  await AiDigestModel.findOneAndUpdate(
    { childId, date },
    {
      childId,
      date,
